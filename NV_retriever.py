...
class ContrastiveDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        # Handle empty strings - replace with placeholder text
        if not query.strip():
            query = "No query available"
        if not positive.strip():
            positive = "No positive response available"
        ...
        if not negative.strip():
            negative = "No negative response available"
...
def get_embeddings(model, input_dict):
    # Debug: print what parameters we're getting
    print(f"Debug - input_dict keys: {list(input_dict.keys())}")
    
    # Filter input_dict to only include the basic parameters that most models support
    # Exclude 'inputs_embeds' and other advanced parameters
    filtered_inputs = {}
    if 'input_ids' in input_dict:
        filtered_inputs['input_ids'] = input_dict['input_ids']
    if 'attention_mask' in input_dict:
        filtered_inputs['attention_mask'] = input_dict['attention_mask']
    
    print(f"Debug - filtered_inputs keys: {list(filtered_inputs.keys())}")
    ...
    try:
        ...
        print(f"Debug - Model forward error: {e}")
