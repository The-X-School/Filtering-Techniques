...
def deduplicate(threshold=0.8, ngram_size=10, num_perm = 128, total_sample = 100000):
    ...
    return dataset, filtered_dataset, len(dataset) - len(filtered_dataset)

def main():
    # Load the processed data   
    original_dataset, filtered_dataset, total_filtered = deduplicate(threshold=0.5, ngram_size=3, num_perm = 32, total_sample = 100000)
    
    print(f"\nReduced from {len(original_dataset)} to {len(filtered_dataset)} documents")
